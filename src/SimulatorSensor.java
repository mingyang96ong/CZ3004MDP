...
public class SimulatorSensor extends Sensor{
	...
	private void updateSensorsValue(int x, int y, String direction) {
		...
		for (int i = Constant.SHORTSENSORMINRANGE; i < Constant.SHORTSENSORMAXRANGE; i ++ ) {
		...
		for (int i = Constant.FARSENSORMINRANGE; i < Constant.FARSENSORMAXRANGE; i ++ ) {
	...
	private String[] padSensorValue(String [] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			if (arr[i] == null) {
				...
				arr[i] = "" + (Constant.FARSENSORMAXRANGE * 10 + 1);
			}
		}
		return arr;
	}
