...
public class Exploration {
        ...
        if (check_right_empty(robot)) {
            ...
            if (check_front_empty(robot)) {
                ...
                if ((checked != null)&&(!front_wall(robot))) {
                    checked = image_recognition(robot, checked_obstacles);
                } else {
                    robot.rotateLeft();
                }
            }
    ...
    private int[][] image_recognition(Robot robot, int[][] checked_obstacles) {
        ...
        for (int i=0; i<3; i++) {
            if ((within_map(obs_pos[i][0], obs_pos[i][1])) && (x != obs_pos[i][0]) && (y != obs_pos[i][1])) {
        ...
        for (int j=0; j<3; j++) {
            if (!(Arrays.equals(obs_pos[j], default_pos))) {
                take_pic = true;
                if (checked_obstacles.length == 0) {
                    checked_obstacles = new int[][] {obs_pos[j]};
                } else {
                    int[][] temp = new int[checked_obstacles.length + 1][3];
                    System.arraycopy(checked_obstacles, 0, temp, 1, checked_obstacles.length);
                    temp[0] = obs_pos[j];
        ...
        for (int i=0; i<Constant.BOARDWIDTH; i++) {
            for (int j=0; j<Constant.BOARDHEIGHT; j++) {
                if (map.getGrid(i,j).equals(Constant.OBSTACLE)) {
                    ...
                    for (int[] obstacles : checked_obstacles) {
                        int[] o_pos = {obstacles[0], obstacles[1]};
                        int[] cur = {i, j};
